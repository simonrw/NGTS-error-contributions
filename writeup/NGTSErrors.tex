...
\[
    F = \frac{\int_{-0.25}^{0.25}\int_{-0.25}^{0.25} G dx dy}
    {\int_{-\infty}^{\infty}\int_{-\infty}^{\infty} G dx dy}
    \]

where $G$ is a 2D Gaussian function centered on $0$ with a width of
$\sigma$, 
...
contributing. The fits are displayed on fig.~\ref{fig:satvsexpos} but 
...
in a list of filters (Johnson-Cousins BRIJHK) but I is the closest match 
to the NGTS filter. 

%TODO Try with R band 
